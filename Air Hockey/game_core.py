...
class GameCore:
    ...
    def update_game_state(self):
        ...
        if self.puck_pos['y'] <= self.puck_radius or self.puck_pos['y'] >= self.board_height - self.puck_radius:
        ...
        # Check for game over
        if self.goals['left'] >= self.max_goals or self.goals['right'] >= self.max_goals:
            self.running = False
    ...
    def reset_puck(self, side):
        ...
        self.puck_speed = {'x': 5 if side == 'left' else -5, 'y': 3}

    def move_paddle(self, player, x_pos, y_pos):
        if player == 1:
            self.paddle1_velocity = {'x': x_pos - self.paddle1_pos['x'], 'y': y_pos - self.paddle1_pos['y']}
            self.paddle1_pos['x'] = max(self.paddle_radius, min(x_pos, self.board_width // 2 - self.paddle_radius))
            self.paddle1_pos['y'] = max(self.paddle_radius, min(y_pos, self.board_height - self.paddle_radius))
        elif player == 2:
            self.paddle2_velocity = {'x': x_pos - self.paddle2_pos['x'], 'y': y_pos - self.paddle2_pos['y']}
            self.paddle2_pos['x'] = max(self.board_width // 2 + self.paddle_radius,
                                        min(x_pos, self.board_width - self.paddle_radius))
            self.paddle2_pos['y'] = max(self.paddle_radius, min(y_pos, self.board_height - self.paddle_radius))
